...
> **提示**
> 因为ROC曲线和准确率/召回率（PR）曲线很相似，你可能想知道该如何决定用哪个。根据经验，每当正例很稀少，或你更关注假正例时，你应该更偏好PR曲线。其他情况用ROC曲线。例如，看之前的ROC曲线（和ROC AUC数值），你可能会觉得这个分类器很棒。但这主要是因为和反例（非5）相比，正例（5）很少。相反地，PR曲线就能清楚表明，分类器还有提升空间（曲线还能更靠近左上角）。
